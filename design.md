...
| `.textarea` / `.textarea.focus` | 1px `--border-strong`, `--radius-sm`, 10×14; focus = primary border + 3px `--primary-soft` halo | Same recipe for `<input>` — add `.single` for one line (44px, centred) |
| `.fg` / `.fg-label` / `.fg-hint` | field group: label row (14/500 `--text`, optional marker `.opt` 13 `--text-light`) · 4px · hint 13/1.45 `--text-muted` · 8px · control · 8px · `.inline-error`. 24px to the next `.fg` | The unit every form field is built from. Exactly one hint line per field, and the hint explains the field — the label never repeats it |
| Field states | on `.textarea`: default `--border-strong` · `.focus` primary + 3px `--primary-soft` · `.error` 1px `--peach-text` · `.error.focus` `--peach-text` + 3px `--peach-soft` · `.is-disabled` `--surface` fill, `--border`, `--text-muted` ink | The error focus ring is the focus recipe re-run in the error colour. `.is-disabled` on a field keeps **full opacity** (unlike a button) — the recessed fill is the signal; add it to the `.fg` too to demote label and hint one level |
| `.inline-error` | 10px gap, 12×14, `--radius-sm`, `--peach-soft` bg, `--peach-text`, alert icon | Field-level message under a `.fg`, form-level message at the top of the card, or standalone. Never a modal or toast for a failed action |
